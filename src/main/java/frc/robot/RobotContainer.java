...
public class RobotContainer {
	...
	/*
	 * @Log private final ControlPanelSubsystem m_controlpanel = new
	...
	public RobotContainer() {
		...
		m_climb.setDefaultCommand(
				...
				new RunCommand(
						() -> m_climb.setOutput(
								Math.max(m_operatorController.getRawAxis(2), m_driverController.getRawAxis(2)),
								Math.max(m_operatorController.getRawAxis(3), m_driverController.getRawAxis(3))),
						m_climb));
	...
	private void configureButtonBindings() {
		...
		new JoystickButton(m_driverController, XboxController.Button.kX.value)
				...
				.whenActive(new InstantCommand(m_conveyor::turnBackwards).andThen(new WaitCommand(.15)
						.andThen(new InstantCommand(m_conveyor::turnOff).andThen(new InstantCommand(() -> {
		...
		// Turn on the conveyor when either the button is pressed or if the bottom
		// sensor is blocked
		...
		// When right bumper is pressed raise/lower the intake and stop/start the intake
		// on both controllers
		 ...
		 * .or(new JoystickButton(m_operatorController,
		 * XboxController.Button.kBack.value)) .whenActive(new DriveStraight(120,
		 * m_robotDrive).withTimeout(10));
		 */

		// When the left bumper is pressed on either controller go to the next climber
		// stage
		...
		new JoystickButton(m_driverController, XboxController.Button.kY.value)
				...
				.whileHeld(
						new InstantCommand(() -> m_Limelight.turnToTargetVolts(m_robotDrive, m_shooter), m_robotDrive))
		...
		// When Y button is pressed on operators controller deploy the intake but do not
		// spin the wheels
		new JoystickButton(m_operatorController, XboxController.Button.kY.value)
				.whenPressed(new InstantCommand(() -> m_intake.toggleIntakePosition(true)));

		// Create "button" from POV Hat in up direction. Use both of the angles to the
		// left and right also.
		new POVButton(m_driverController, 315).or(new POVButton(m_driverController, 0))
				.or(new POVButton(m_driverController, 45)).whenActive(new TurnToAngle(90, m_robotDrive).withTimeout(5));

		// Create "button" from POV Hat in down direction. Use both of the angles to the
		// left and right also.
		new POVButton(m_driverController, 225).or(new POVButton(m_driverController, 180))
				.or(new POVButton(m_driverController, 135))
				.whenActive(new TurnToAngle(-90, m_robotDrive).withTimeout(5));

		// POV Up Direction on Operator Controller relatively increases the current
		// setpoint of the shooter
		new POVButton(m_operatorController, 315).or(new POVButton(m_operatorController, 0))
				.or(new POVButton(m_operatorController, 45)).whenActive(new InstantCommand(() -> {
					m_shooter.setSetpoint(m_shooter.getSetpoint() + 50);
				}));

		// POV Down Direction on Operator Controller relatively increases the current
		// setpoint of the shooter
		new POVButton(m_operatorController, 225).or(new POVButton(m_operatorController, 180))
				.or(new POVButton(m_operatorController, 135)).whenActive(new InstantCommand(() -> {
					m_shooter.setSetpoint(m_shooter.getSetpoint() - 50);
				}));
	}

	/*
	 * public void LimelightCamera() { // Activate an HttpCamera for the Limelight
	 * m_limelightFeed = new HttpCamera("Limelight Camera",
	 * "http://10.63.91.11:5800/stream.mjpg",
	 * HttpCamera.HttpCameraKind.kMJPGStreamer); }
	 * 
	 * @Log.CameraStream(name = "Limelight Camera", tabName = "Dashboard") public
	 * HttpCamera getLimelightFeed() { return m_limelightFeed; }
	 */
