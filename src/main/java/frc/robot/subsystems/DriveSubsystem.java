...
public class DriveSubsystem extends SubsystemBase implements Loggable {
	...
	private final SimpleMotorFeedforward m_driveFeedforward = new SimpleMotorFeedforward(DriveConstants.kS,
	...
	public DriveSubsystem() {
		...
		m_talonsrxright.configSelectedFeedbackSensor(FeedbackDevice.RemoteSensor1, // Set remote sensor to be used
																					// directly
		 ...
		 * Constants.java
		 */
		m_talonsrxright.configSelectedFeedbackCoefficient(
				DriveConstants.kTurnTravelUnitsPerRotation / DriveConstants.kPigeonUnitsPerRotation, // Coefficient
		...
		m_talonsrxright.config_kP(DriveConstants.kSlot_Distanc, DriveConstants.kGains_Distanc.kP,
				DriveConstants.kTimeoutMs);
		m_talonsrxright.config_kI(DriveConstants.kSlot_Distanc, DriveConstants.kGains_Distanc.kI,
				DriveConstants.kTimeoutMs);
		m_talonsrxright.config_kD(DriveConstants.kSlot_Distanc, DriveConstants.kGains_Distanc.kD,
				DriveConstants.kTimeoutMs);
		m_talonsrxright.config_kF(DriveConstants.kSlot_Distanc, DriveConstants.kGains_Distanc.kF,
				DriveConstants.kTimeoutMs);
		m_talonsrxright.config_IntegralZone(DriveConstants.kSlot_Distanc, DriveConstants.kGains_Distanc.kIzone,
				DriveConstants.kTimeoutMs);
		m_talonsrxright.configClosedLoopPeakOutput(DriveConstants.kSlot_Distanc,
				DriveConstants.kGains_Distanc.kPeakOutput, DriveConstants.kTimeoutMs);

		/* FPID Gains for turn servo */
		m_talonsrxright.config_kP(DriveConstants.kSlot_Turning, DriveConstants.kGains_Turning.kP,
				DriveConstants.kTimeoutMs);
		m_talonsrxright.config_kI(DriveConstants.kSlot_Turning, DriveConstants.kGains_Turning.kI,
				DriveConstants.kTimeoutMs);
		m_talonsrxright.config_kD(DriveConstants.kSlot_Turning, DriveConstants.kGains_Turning.kD,
				DriveConstants.kTimeoutMs);
		m_talonsrxright.config_kF(DriveConstants.kSlot_Turning, DriveConstants.kGains_Turning.kF,
				DriveConstants.kTimeoutMs);
		m_talonsrxright.config_IntegralZone(DriveConstants.kSlot_Turning, DriveConstants.kGains_Turning.kIzone,
				DriveConstants.kTimeoutMs);
		m_talonsrxright.configClosedLoopPeakOutput(DriveConstants.kSlot_Turning,
	...
	/**
	 * Resets the current pose to the specified pose. This this ONLY be called when
	 * the robot's position on the field is known, like at the beginnig of a match.
	 * This will also reset the saved pose since the old pose could be invalidated.
	 * 
	 ...
	 */
	/*
	 * public DifferentialDriveWheelSpeeds getWheelSpeeds() { return new
	 * DifferentialDriveWheelSpeeds( m_talonsrxleft.getSelectedSensorVelocity() *
	 * DriveConstants.kEncoderDistancePerPulse,
	 * m_talonsrxright.getSelectedSensorVelocity() *
	 * DriveConstants.kEncoderDistancePerPulse); }
	 */
	...
	/**
	 * Attempts to follow the given drive states using offboard PID. THIS IS
	 * UNTESTED
	 *
	...
	public void setDriveStates(TrapezoidProfile.State left, TrapezoidProfile.State right) {
		m_talonsrxleft.set(ControlMode.Position, left.position, DemandType.ArbitraryFeedForward,
				m_driveFeedforward.calculate(left.velocity));
		m_talonsrxright.set(ControlMode.Position, right.position, DemandType.ArbitraryFeedForward,
	...
	public void tankDriveVelocity(double leftVelocity, double rightVelocity) {
		...
		var rightAccel = (rightVelocity - stepsPerDecisecToMetersPerSec(m_talonsrxright.getSelectedSensorVelocity()))
				/ 20;
		...
		m_talonsrxleft.set(ControlMode.Velocity, metersPerSecToStepsPerDecisec(leftVelocity),
				DemandType.ArbitraryFeedForward, leftFeedForwardVolts / 12);
		m_talonsrxright.set(ControlMode.Velocity, metersPerSecToStepsPerDecisec(rightVelocity),
	...
	public Command createCommandForTrajectory(String trajname) {
		...
		return new RamseteCommand(trajectory, this::getCurrentPose,
				...
				DriveConstants.kDriveKinematics, this::tankDriveVelocity, this);
	}
