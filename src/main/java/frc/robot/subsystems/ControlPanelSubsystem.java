...
public class ControlPanelSubsystem extends SubsystemBase implements Loggable {
	...
	enum State {
		DISABLED, ENC_ROTATE, COLOR_ROTATE, COLOR_ROTATE_FINAL,
	};
	...
	public void StartColorFind(double joystickval) {
		...
		if (index >= 4) {
			...
			// need to rotate around the table. We look at it from the far left, and need to
			// rotate it 3 slots left, or 1 right, over to be what needs to be under the
			// sensor.
